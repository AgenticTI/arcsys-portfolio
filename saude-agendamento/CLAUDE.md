...
There is no auth. The entry point (`/`) is a dark-themed landing page with navbar, hero section, features strip, and two CTA buttons — "Sou Paciente" navigates to `/paciente/dashboard`, "Sou Médico" navigates to `/medico/agenda`. The logged-in identities are hardcoded in `src/data/patient.json` and `src/data/doctor-user.json`.
...
src/
  app/
    page.tsx                        # Dark landing page with hero + features (/)
    layout.tsx                      # Root layout — sets <html lang="pt-BR">
    globals.css                     # Tailwind directives + Inter font import

    paciente/
      layout.tsx                    # Responsive: Sidebar (desktop) + MobileHeader + MobileTabBar (mobile)
    ...
    medico/
      layout.tsx                    # DoctorSidebar (desktop) + dark mobile header
      agenda/page.tsx               # Doctor dashboard: stats + schedule table + calendar + chart

  components/
    Sidebar.tsx                     # Patient dark sidebar (desktop only, hidden on mobile)
    MobileTabBar.tsx                # Patient bottom tab bar for mobile (Home, Buscar, FAB, Histórico, Perfil)
    MobileHeader.tsx                # Patient mobile header (greeting on dashboard, page title on others)
    DoctorSidebar.tsx               # Doctor icon-only sidebar (64px, dark theme, desktop only)
    StatsCard.tsx                   # Reusable stat card (icon + label + value + trend)
    ScheduleTable.tsx               # Doctor daily schedule table with confirm/cancel actions
    MiniCalendar.tsx                # Monthly calendar widget (dark theme)
    WeeklyChart.tsx                 # Pure CSS bar chart for weekly bookings
    AppointmentCard.tsx             # Appointment row used in dashboard + historico
    DoctorCard.tsx                  # Doctor card (centered grid desktop, horizontal row mobile)
    StatusBadge.tsx                 # Pill badge for confirmed / pending / cancelled
    TimeSlotGrid.tsx                # Responsive time slot grid (2 cols mobile, 4 cols desktop)
...
### Two roles, no auth
The landing page (`src/app/page.tsx`) has two CTA buttons ("Sou Paciente" / "Sou Médico"). There is no session, cookie, or context. The "logged-in" patient is always `patient.json` (`id: "p1"`, Rafael Mendes) and the "logged-in" doctor is always `doctor-user.json` (`id: "d3"`, Dr. Carlos Lima).

### Dual theme strategy
The app uses two visual themes based on role:
- **Patient pages** (`/paciente/*`): Light theme — `bg-neutral-100` page background, white cards, dark sidebar (240px)
- **Doctor pages** (`/medico/*`): Dark theme — `bg-dark-bg` page background, `bg-dark-card` cards, icon sidebar (64px)
- **Landing page** (`/`): Dark theme

### Mobile navigation
On screens below `md` (768px):
- Patient sidebar is hidden, replaced by `MobileTabBar` (bottom tab bar with FAB) + `MobileHeader`
- Doctor top header is replaced by a compact dark mobile header
- Main content has `pb-20` padding to clear the fixed tab bar

### Route namespacing via layouts
- `/paciente/*` routes share `src/app/paciente/layout.tsx` which renders `<Sidebar>` (desktop, hidden on mobile), `<MobileHeader>` (mobile only), and `<MobileTabBar>` (mobile only, fixed bottom).
- `/medico/*` routes share `src/app/medico/layout.tsx` which renders `<DoctorSidebar>` (desktop, 64px icon sidebar) and a compact dark mobile header.
...
### TODAY is hardcoded
`const TODAY = "2026-03-17"` appears in `dashboard/page.tsx`, `historico/page.tsx`, and `medico/agenda/page.tsx`. In `agendar/[id]/page.tsx` the equivalent is `const today = new Date("2026-03-17")` (a Date object, lowercase name). This is intentional — the mock data dates are relative to this value. Do not replace it with `new Date()` unless you also update all appointment dates in `appointments.json`.
...
- `MobileTabBar.tsx` — client (uses `usePathname`)
- `MobileHeader.tsx` — client (uses `usePathname`)
- `DoctorSidebar.tsx` — client (imports JSON directly)
...
- `bg-primary` / `text-primary` → lime `#88CD0A`
- `bg-primary/10` → lime with 12% opacity (light accent backgrounds)
- `text-primary-dark` → `#6B9C00` (text on light primary backgrounds)
- `bg-dark-bg` → `#0f1114` (dark page background)
- `bg-dark-card` → `#1A1D23` (dark card background)
- `bg-dark-border` → `#2d3139` (dark borders and surfaces)
- `bg-dark-surface` → `#3d4149` (elevated dark surfaces)
- `text-neutral-900` / `text-neutral-500` / `bg-neutral-100` → the standard neutral scale
- `shadow-card` → `0 1px 4px rgba(0,0,0,0.05)`
- `rounded-xl` for cards, `rounded-lg` for buttons/inputs, `rounded-[14px]` for larger containers
...
3. **Add a nav item to the Sidebar and MobileTabBar:** open `src/components/Sidebar.tsx` and add an entry to the `navItems` array, then open `src/components/MobileTabBar.tsx` and add a tab to the `tabs` array.
...
6. **Use existing design tokens** — stick to `bg-primary`, `shadow-card`, `rounded-xl`, `text-neutral-*`, and `dark-*` tokens as used in the rest of the app. Patient pages use light theme, doctor pages use dark theme. Do not introduce new colors without adding them to `tailwind.config.ts`.

### Example: add a new doctor page (e.g. "Patient Detail")

1. Create `src/app/medico/<route>/page.tsx`.
2. The doctor layout (`src/app/medico/layout.tsx`) wraps it automatically with `DoctorSidebar` (desktop) and dark mobile header. Use `dark-*` tokens for all doctor pages.
